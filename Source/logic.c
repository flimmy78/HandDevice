...
U8 logic_readGPRSParam(U8* gatewayId, U8* apnId, U8* svrId, U8* port)
{
	...
	Lib_sprintf((char*)apnId, "%d", paramStr.u8APN);
	Lib_sprintf((char*)port, "%d", lu16port);
	Lib_sprintf((char*)svrId, "%d.%d.%d.%d", \
...
U8 logic_readVersion(U8* gatewayId, U8* hardVer, U8* softVer)
{
	...
	if (logic_sendAndRead(buf, &bufSize, UART_WAIT_SHORT) == ERROR)
		return ERROR;
	protoA_GPRSParam(buf, bufSize, &paramStr);
	Lib_sprintf((char*)hardVer, "%02d.%02d", paramStr.HardwareVer[1], paramStr.HardwareVer[0]);
	Lib_sprintf((char*)softVer, "%02d.%02d", paramStr.SoftVer[1], paramStr.SoftVer[0]);
	return NO_ERR;
}
...
U8 logic_saveConfig(U8  device, U32 baud, U8  mode, U8* gatewayId, U8* svrId)
{
	if (db_setComConfig(device, baud, mode) == ERROR) {
		return ERROR;
	}
	if (db_setGatewayId(gatewayId) == ERROR) {
		return ERROR;
	}
	if (db_setSvrId(svrId) == ERROR) {
		return ERROR;
	}
